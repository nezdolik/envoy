...
protected:
  ...
  void initializeTest(bool http_active_hc, uint32_t num_clusters_to_add,
    ...
    if (edsUpdateMode() == Grpc::EdsUpdateMode::Multiplexed) {
      ...
      for (uint32_t i = 0; i < num_clusters_to_add; ++i) {
        initXdsStream();
        expected_requests_contents.push_back({Config::TypeUrl::get().ClusterLoadAssignment, {"cluster_"+std::to_string(i)}, {}, Grpc::Status::WellKnownGrpcStatus::Ok, ""});
  ...
  while (curr_stream_idx < xds_streams_.size()) {
    for (auto it = expected_requests_contents.begin(); it != expected_requests_contents.end(); ++it){
      ...
      if (sotw_or_delta_ == Grpc::SotwOrDelta::Sotw) {
        ...
        VERIFY_ASSERTION(xds_streams_[curr_stream_idx]->waitForGrpcMessage(*dispatcher_, request));
...
AssertionResult assertExpectedDiscoveryRequest(const envoy::service::discovery::v3::DiscoveryRequest& request, const DiscoveryRequestExpectedContents& expected_request){
...
AssertionResult assertExpectedDeltaDiscoveryRequest(const envoy::service::discovery::v3::DeltaDiscoveryRequest& request, const DiscoveryRequestExpectedContents& expected_request){
    if (!request.has_node() || request.node().id().empty() || request.node().cluster().empty()) {
      return AssertionFailure() << "Weird node field";
    }
...
AssertionResult compareSets(const std::set<std::string>& set1, const std::set<std::string>& set2,
  ...
  void acceptXdsConnection() {
    ...
    if (edsUpdateMode() == Grpc::EdsUpdateMode::Multiplexed){
      // Reuse single default xds stream.
