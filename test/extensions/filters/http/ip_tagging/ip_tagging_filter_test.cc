...
class IpTaggingFilterTest : public Event::TestUsingSimulatedTime, public ::testing::TestWithParam<std::string> {
public:
  IpTaggingFilterTest() : api_(Api::createApiForTest()), dispatcher_(api_->allocateDispatcher("test_thread")){
    ...
    for (int i = 0; i < seconds; i++) {
      simTime().advanceTimeAndRun(std::chrono::seconds(1), *dispatcher_,
...
ip_tags_datasource:
  ...
  initializeFilter(config_yaml,
                   "INVALID_ARGUMENT: Unsupported file format, unable to parse ip tags from file /test/tags.csv");
}
...
TEST_F(IpTaggingFilterTest, InternalRequestWithReloadMaybeWorks) {
...
ip_tags_datasource:
    ...
    TestEnvironment::writeStringToFileForTest(TestEnvironment::temporaryPath("ip_tagging_test/watcher_target.yaml"), R"EOF(
ip_tags:
  - ip_tag_name: internal_request
    ip_list:
      - {address_prefix: 1.2.3.5, prefix_len: 32}
                                            ...
                                            true);
  // TestEnvironment::createSymlink(TestEnvironment::temporaryPath("envoy_test/watcher_target.yaml"),
  //                                TestEnvironment::temporaryPath("envoy_test/watcher_link"));


  TestEnvironment::writeStringToFileForTest(TestEnvironment::temporaryPath("ip_tagging_test/watcher_new_target.yaml"), R"EOF(
ip_tags:
  - ip_tag_name: internal_updated_request
    ip_list:
      - {address_prefix: 1.2.3.5, prefix_len: 32}
                                            ...
                                            true);
  // TestEnvironment::createSymlink(TestEnvironment::temporaryPath("envoy_test/watcher_new_target.yaml"),
  ...
  TestEnvironment::renameFile(TestEnvironment::temporaryPath("ip_tagging_test/watcher_target.yaml"),
                              TestEnvironment::temporaryPath("ip_tagging_test/watcher_old_target.yaml"));
  TestEnvironment::renameFile(TestEnvironment::temporaryPath("ip_tagging_test/watcher_new_target.yaml"),
                              TestEnvironment::temporaryPath("ip_tagging_test/watcher_target.yaml"));

  auto contents = TestEnvironment::readFileToStringForTest(TestEnvironment::temporaryPath("ip_tagging_test/watcher_target.yaml"));
  std::cerr << "****Content of renamed file:" << contents << std::endl;
  // Handle the events if any.
  dispatcher_->run(Event::Dispatcher::RunType::NonBlock);

  simTime().advanceTimeWait(std::chrono::seconds(6));

  std::cerr << "****Before next decode Headers!!!" << std::endl;
