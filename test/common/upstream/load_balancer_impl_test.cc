...
public:
  ...
  static bool isSlowStartEnabled(EdfLoadBalancerBase& edf_lb) {
    return edf_lb.slow_start_enabled_;
  }
  static const std::chrono::milliseconds latestHostAddedTime(EdfLoadBalancerBase& edf_lb) {
    return std::chrono::time_point_cast<std::chrono::milliseconds>(edf_lb.latest_host_added_time_).time_since_epoch();
  }
};
...
TEST_P(RoundRobinLoadBalancerTest, SlowStartWithDefaultParams) {
  ...
  const auto slow_start_enabled_ = EdfLoadBalancerBasePeer::isSlowStartEnabled(static_cast<EdfLoadBalancerBase&>(*lb_));
  EXPECT_EQ(false, slow_start_enabled_);
  const auto latest_host_added_time = EdfLoadBalancerBasePeer::latestHostAddedTime(static_cast<EdfLoadBalancerBase&>(*lb_));
...
TEST_P(RoundRobinLoadBalancerTest, SlowStartNoWait) {
  ...
  auto latest_host_added_time_ms = EdfLoadBalancerBasePeer::latestHostAddedTime(static_cast<EdfLoadBalancerBase&>(*lb_));
  ...
  latest_host_added_time_ms = EdfLoadBalancerBasePeer::latestHostAddedTime(static_cast<EdfLoadBalancerBase&>(*lb_));
...
TEST_P(RoundRobinLoadBalancerTest, SlowStartWithRuntimeAggression) {
  ...
  // We should see 1:1:1:0 ratio, as host 2 and 3 weight is scaled with (9/10)^(1/1.5)=0.93 factor, host4 weight is 0.002.
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_->chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[1], lb_->chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[2], lb_->chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_->chooseHost(nullptr));
  ...
  // We should see 1:1:1:1 ratio, only host4 is in slow start with weight 0.93, and the rest of hosts are outside of slow start with weight 1.
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_->chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[1], lb_->chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[2], lb_->chooseHost(nullptr));
...
TEST_P(LeastRequestLoadBalancerTest, SlowStartWithDefaultParams) {
  ...
  EXPECT_EQ(1.0, aggression);
  const auto slow_start_enabled_ = EdfLoadBalancerBasePeer::isSlowStartEnabled(static_cast<EdfLoadBalancerBase&>(lb_2));
  EXPECT_EQ(false, slow_start_enabled_);
  const auto latest_host_added_time = EdfLoadBalancerBasePeer::latestHostAddedTime(static_cast<EdfLoadBalancerBase&>(lb_2));
  EXPECT_EQ(std::chrono::milliseconds(0), latest_host_added_time);  
}
...
TEST_P(LeastRequestLoadBalancerTest, SlowStartNoWait) {
  ...
  auto latest_host_added_time = EdfLoadBalancerBasePeer::latestHostAddedTime(static_cast<EdfLoadBalancerBase&>(lb_2));
  ...
  latest_host_added_time = EdfLoadBalancerBasePeer::latestHostAddedTime(static_cast<EdfLoadBalancerBase&>(lb_2));
  ...
  // We expect 3:2 ratio, as host2 is in slow start mode and it's weight is scaled with
  // 0.33 factor and host1 weight with 0.5 factor (due to active request bias).
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_2.chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_2.chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[1], lb_2.chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_2.chooseHost(nullptr));
  ...
  // host2 is 50 secs in slow start, the weight is scaled with time factor 50 / 60 == 0.83.
  simTime().advanceTimeWait(std::chrono::seconds(30));

  // Recalculate weights.
  hostSet().runCallbacks({}, {});

  // We expect 5:3 ratio, as host2 is in slow start mode and it's weight is scaled with
  // 0.83 factor and host1 weight with 0.5 factor.
  EXPECT_EQ(hostSet().healthy_hosts_[1], lb_2.chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[0], lb_2.chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[1], lb_2.chooseHost(nullptr));
  EXPECT_EQ(hostSet().healthy_hosts_[1], lb_2.chooseHost(nullptr));
...
TEST_P(LeastRequestLoadBalancerTest, SlowStartWaitForPassingHC) {
  ...
  // We expect 6:5 ratio, as host2 is in slow start mode, its weight is scaled with factor
  // pow(0.4, 1.11)=0.36. Host1 is 6 seconds in slow start and its weight is scaled with active
  // request and time bias 0.5 * pow(0.6, 1.11) = 0.28.
