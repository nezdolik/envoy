...
public:
  IpTagsProvider(const envoy::config::core::v3::DataSource& ip_tags_datasource, IpTagsLoader& tags_loader,
...
public:
  ...
  std::shared_ptr<IpTagsProvider> get(const envoy::config::core::v3::DataSource& ip_tags_datasource, IpTagsLoader& tags_loader,
                                      uint64_t ip_tags_refresh_interval_ms, Api::Api& api, ThreadLocal::SlotAllocator& tls,
    ...
    if (it != ip_tags_registry_.end()) {
      if (std::shared_ptr<IpTagsProvider> provider = it->second.lock()) {
        ...
        ip_tags_provider = std::make_shared<IpTagsProvider>(
            ip_tags_datasource, tags_loader, ip_tags_refresh_interval_ms, main_dispatcher, api, tls,
            singleton, creation_status);
        ip_tags_registry_[key] = ip_tags_provider;
      }
    } else {
      ip_tags_provider = std::make_shared<IpTagsProvider>(
          ip_tags_datasource, tags_loader, ip_tags_refresh_interval_ms, main_dispatcher, api, tls, singleton,
          creation_status);
      ip_tags_registry_[key] = ip_tags_provider;
    }
...
public:
  ...
  const Network::LcTrie::LcTrie<std::string>& trie() const {
    if (provider_) {
      std::cerr << "***calling provider get tags"<<std::endl;
