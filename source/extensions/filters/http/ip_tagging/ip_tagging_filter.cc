...
IpTagsProvider::IpTagsProvider(const envoy::config::core::v3::DataSource& ip_tags_datasource, IpTagsLoader& tags_loader,
                               uint64_t ip_tags_refresh_interval_ms, Event::Dispatcher& dispatcher,
                               Api::Api& api, ThreadLocal::SlotAllocator& tls, Singleton::InstanceSharedPtr owner,
    ...
    //ip_tags_refresh_interval_(PROTOBUF_GET_MS_OR_DEFAULT(config, ip_tags_refresh_interval_ms_, 0)),
    owner_(owner) {
  ...
  if (ip_tags_refresh_interval_ms_ > std::chrono::milliseconds(0) && ip_tags_datasource.has_watched_directory()) {
    needs_refresh_ = true;
  }
  tags_ = tags_loader_.loadTags(ip_tags_datasource, dispatcher, api, tls, creation_status);
}
...
LcTrieSharedPtr IpTagsProvider::ipTags() ABSL_LOCKS_EXCLUDED(ip_tags_mutex_) {
  std::cerr << "****Calling getting tags!!!" << std::endl;
  if (needs_refresh_) {
      const auto current_time = time_source_.monotonicTime();
      if (std::chrono::duration_cast<std::chrono::milliseconds>(
        current_time - last_reloaded_time_) > ip_tags_refresh_interval_ms_) {
          absl::Status refresh_status = absl::OkStatus();
          absl::MutexLock lock(&ip_tags_mutex_);
          tags_ = tags_loader_.refreshTags(refresh_status);
          last_reloaded_time_ = current_time;
          std::cerr << "****Reloaded tags!!!" << std::endl;
          //todo do smth with refresh status
      }
  }
...
IpTagsLoader::IpTagsLoader(Api::Api&, ProtobufMessage::ValidationVisitor& validation_visitor,
                           Stats::StatNameSetPtr& stat_name_set)
    : validation_visitor_(validation_visitor), stat_name_set_(stat_name_set) {}

LcTrieSharedPtr IpTagsLoader::loadTags(const envoy::config::core::v3::DataSource& ip_tags_datasource,
                                       Event::Dispatcher& dispatcher, Api::Api& api, ThreadLocal::SlotAllocator& tls, absl::Status& creation_status) {
  if (!ip_tags_datasource.filename().empty()) {
    if (!absl::EndsWith(ip_tags_datasource.filename(), MessageUtil::FileExtensions::get().Yaml) &&
      ...
      creation_status =
          absl::InvalidArgumentError("Unsupported file format, unable to parse ip tags from file " + ip_tags_datasource.filename());
      return nullptr;
    }
    auto provider_or_error = Config::DataSource::DataSourceProvider::create(
        ip_tags_datasource, dispatcher, tls, api, false, 0);
    if (!provider_or_error.ok()) {
      creation_status = absl::InvalidArgumentError(fmt::format("unable to create data source '{}'",
...
LcTrieSharedPtr IpTagsLoader::refreshTags(absl::Status& refresh_status) {
  if (data_source_provider_) {
    std::cerr << "****data_source data after reload: " << data_source_provider_->data() << std::endl;
    IpTagFileProto ip_tags_proto;
    if (absl::EndsWith(ip_tags_path_, MessageUtil::FileExtensions::get().Yaml)) {
      TRY_NEEDS_AUDIT {
        MessageUtil::loadFromYaml(data_source_provider_->data(), ip_tags_proto, validation_visitor_);
      }
      ...
      refresh_status = MessageUtil::loadFromJsonNoThrow(data_source_provider_->data(), ip_tags_proto,
                                                          has_unknown_field);
...
LcTrieSharedPtr IpTagsLoader::parseIpTagsAsProto(
  ...
  for (const auto& ip_tag : ip_tags) {
    std::cerr << "ip_tag: " << ip_tag.ip_tag_name() << std::endl;
...
absl::StatusOr<IpTaggingFilterConfigSharedPtr> IpTaggingFilterConfig::create(
    ...
    Runtime::Loader& runtime, Api::Api& api, ThreadLocal::SlotAllocator& tls, Event::Dispatcher& dispatcher,
...
IpTaggingFilterConfig::IpTaggingFilterConfig(
  ...
  if (!config.ip_tags().empty()) {
    ...
    provider_ = ip_tags_registry_->get(
        config.ip_tags_datasource(), tags_loader_, ip_tags_refresh_interval_ms, api, tls, dispatcher, ip_tags_registry_, creation_status);
    RETURN_ONLY_IF_NOT_OK_REF(creation_status);
...
Http::FilterHeadersStatus IpTaggingFilter::decodeHeaders(Http::RequestHeaderMap& headers, bool) {
  ...
  std::cerr << "****Calling decodeHeaders!!!" << std::endl;
