...
// Actual result of the lookup. Each entry in the map represents the geolocation header (entry key) for which the lookup was invoked
// mapped to a lookup result from the database. Entry value will be set to absl::nullopt when database lookup yields an empty result.
...
class LookupRequest {
  ...
  LookupRequest(Network::Address::InstanceConstSharedPtr&& remote_address, absl::flat_hash_set<std::string>&& geo_headers, absl::flat_hash_set<std::string>&& geo_anon_headers):
  remote_address_(std::move(remote_address)), geo_headers_(std::move(geo_headers)), geo_anon_headers_(std::move(geo_anon_headers)) {};
