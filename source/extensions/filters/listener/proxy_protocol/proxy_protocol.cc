    ...
    if (nullptr != key_value_pair) {
      if (Runtime::runtimeFeatureEnabled(
        "envoy.reloadable_features.use_typed_metadata_in_proxy_protol_listener")) {
          ...
          if (typed_proxy_filter_metadata == filter_metadata.end()) {
            ...
            cb_->setDynamicMetadata(metadata_key, typed_metadata);
          } else {
            (typed_proxy_filter_metadata->second().mutable_typed_metadata())[key_value_pair->key()] = tlv_byte_value;
          }
      } else {
